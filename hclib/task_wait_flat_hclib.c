...
void entrypoint(void *arg) {
    ...
    const unsigned long long nb_start_time = current_time_ns();
    ...
    const unsigned long long nb_end_time = current_time_ns();
    printf("METRIC task_wait_flat %d %.20f\n", N_FLAT_TASK_WAITS,
            (double)N_FLAT_TASK_WAITS / ((double)(nb_end_time -
                    nb_start_time) / 1000.0));

    const unsigned long long blocking_start_time = current_time_ns();
    for (i = 0; i < N_FLAT_TASK_WAITS; i++) {
        hclib_start_finish();
        {
    ...
    const unsigned long long blocking_end_time = current_time_ns();
    printf("METRIC task_wait_flat %d %.20f\n", N_FLAT_TASK_WAITS,
